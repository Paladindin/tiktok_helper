...
object CommandUtils {
    ...
    private suspend fun starAndSendMessage(isStar: Boolean, isMessage: Boolean) {
        if (isStar) {
            var startText = AutoServiceUtils.findViewByContainsText(service, "回关")
            if (startText.isNullOrEmpty()) {
                startText = AutoServiceUtils.findViewByContainsText(service, "关注")
            }
            if (!startText.isNullOrEmpty()) {
                AutoServiceUtils.clickView(startText[0])
                sleep(2000)
            }
            val cancelButtonList = AutoServiceUtils.findViewByEqualsText(service, "取消")
            if (!cancelButtonList.isNullOrEmpty()) {
                if (AutoServiceUtils.clickView(cancelButtonList[0]))
                    sleep(1000)
            }
        ...
        if (isMessage) {
            val moreButton =
                AutoServiceUtils.findViewByFirstEqualsContentDescription(service, "更多")
            Log.e("zwb", "moreButton " + (moreButton == null))
            val b = AutoServiceUtils.clickView(moreButton)
            Log.e("zwb", "click moreButton $b")
            sleep(2000)
            val messges = AutoServiceUtils.findViewByEqualsText(service, "发私信")
            messges?.let {
                AutoServiceUtils.clickView(it[0])
                sleep(1800)
            }
            val editText =
                AutoServiceUtils.findViewByFirstClassName(service, Constants.CLASS_NAME_ET)
            editText?.let {
                AutoServiceUtils.setText(it, ConfigManager.instance.getGreetWord())
                sleep(1500)
            }

            val send = AutoServiceUtils.findViewByFirstEqualsContentDescription(service, "发送")
            send?.let {
                AutoServiceUtils.clickView(it)
                sleep(ConfigManager.instance.getCommentInterval())
            }

            AutoServiceUtils.goBack(service)
            sleep(2000)
        }
    }

    private suspend fun sleep(millis: Int = 1000) {
        delay(millis.toLong())
    }
    ...
    suspend fun assignStar(operationTarget: OperationDetail.TYPE?, random: Boolean = false) {
        val rvList =
            AutoServiceUtils.findViewByClassName(service, Constants.CLASS_NAME_RV)
        ...
        if (rv != null) {
            ...
            for (i in indexList) {
                ...
                var listStar = AutoServiceUtils.findViewByContainsText(child, "关注")
                if (listStar.isNullOrEmpty()) {
                    listStar = AutoServiceUtils.findViewByContainsText(child, "回关")
                }
                if (!listStar.isNullOrEmpty()) {
                    AutoServiceUtils.clickView(listStar[0])
                    sleep(1800)
...
//                child.performAction(AccessibilityNodeInfo.ACTION_CLICK)
                AutoServiceUtils.clickView(child)
                sleep(1200)
    ...
    suspend fun recommendStarAndSendMessage(
        ...
        while (true) {
            val starButton = AutoServiceUtils.findViewById(service, Constants.ID_TITLE)
            AutoServiceUtils.clickView(starButton)
            sleep(3000)

            if (operationTarget == OperationDetail.TYPE.FANS_LIST) {
                val fansList = AutoServiceUtils.findViewByEqualsText(service, "粉丝")
                ...
                AutoServiceUtils.clickView(fansList[0])
                sleep(2500)
            } else {
                val starList = AutoServiceUtils.findViewByEqualsText(service, "关注")
                ...
                AutoServiceUtils.clickView(startButton)
                sleep(2500)
            }
    ...
    private suspend fun scrollHomeRecommend() {
        val viewPager =
            AutoServiceUtils.findViewByClassName(service, Constants.CLASS_NAME_VP)
...
//        val searchEditTextList = AutoServiceUtils.findViewByClassName(service, Constants.CLASS_NAME_ET)
...
//            AutoServiceUtils.clickByGesture(service, rect.left + 200,rect.top + 25)
//        } else {
//            ToastUtils.showShort(App.getInstance(), "此功能需要android 7.0以上系统")
//            return
//        }
//        sleep(1000)
//        AutoServiceUtils.setText(searchEditText, ConfigManager.instance.getBatchUserList()[0])
//        sleep(2000)
//        val searchButton = AutoServiceUtils.findViewByEqualsText(service, "搜索")
//        Log.e("----", "searchButton size ${searchButton.size}")
//        if (searchButton.isNullOrEmpty() || searchButton[0] == null) {
//            return
//        }
//        AutoServiceUtils.clickView(searchButton[0])
//        sleep(3000)
//        val rvUserList = AutoServiceUtils.findViewByClassName(service, Constants.CLASS_NAME_RV)
...
//                var listStar = AutoServiceUtils.findViewByEqualsText(child, "关注")
//                if (!listStar.isNullOrEmpty()) {
...
//        }
    ...
    suspend fun keywordStarAndSendMessage(
        ...
        batchUserList.forEach {
            val searchEditTextList =
                AutoServiceUtils.findViewByClassName(service, Constants.CLASS_NAME_ET)
            if (searchEditTextList.isNullOrEmpty()) {
                return
            }
            val searchEditText = searchEditTextList[1] ?: return
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                AutoServiceUtils.clickByGesture(service, searchEditText)
            } else {
                ToastUtils.showShort(App.getInstance(), "此功能需要android 7.0以上系统")
                return
            }
            AutoServiceUtils.setText(searchEditText, it)
            sleep(1000)
    ...
    suspend fun addressBookStarAndSendMessage(isStar: Boolean, isMessage: Boolean) {
        val rvContact =
            AutoServiceUtils.findViewByFirstClassName(service, Constants.CLASS_NAME_RV)
        rvContact ?: return
        for (i in 0 until rvContact.childCount) {
            val child = rvContact.getChild(i) ?: continue
            val clickSuccess = AutoServiceUtils.clickView2(child)
    ...
    private suspend fun commentStarAndSendMessage(isStar: Boolean, isMessage: Boolean) {
        ...
        rv?.let {
            for (i in 0 until it.childCount) {
                ...
                val title = AutoServiceUtils.findViewById(child, Constants.ID_TITLE) ?: continue
                AutoServiceUtils.clickByGesture(service, title)
                sleep(1000)
                //防止弹起当前页面EditText
                val etList =
                    AutoServiceUtils.findViewByClassName(service, Constants.CLASS_NAME_ET)
    ...
    private suspend fun liveStarAndSendMessage(isStar: Boolean, isMessage: Boolean) {
        val rvUserList = AutoServiceUtils.findViewByClassName(service, Constants.CLASS_NAME_RV)
        val tvTabList = AutoServiceUtils.findViewByEqualsText(service, "本场榜")
        var rvIndex = 0
...
//        rv.performAction(AccessibilityNodeInfo.ACTION_SCROLL_FORWARD)
        ...
        for ((i, child) in childList.withIndex()) {
            AutoServiceUtils.clickByGesture(service, child)
            sleep(2000)
            val tvHome = AutoServiceUtils.findViewByEqualsText(service, "主页")
            if (tvHome.isNullOrEmpty()) continue
            AutoServiceUtils.clickView(tvHome[0])
            sleep(2000)
            starAndSendMessage(isStar, isMessage)
            sleep(1000)
            AutoServiceUtils.goBack(service)
    ...
    private suspend fun commentSameCityList() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            AutoServiceUtils.clickByGesture(service, clickX, clickY)
        }else{
            ToastUtils.showShort(App.getInstance(),"此功能需要android7.0")
            return
        }
        sleep(2000)
        val etList = AutoServiceUtils.findViewByClassName(service, Constants.CLASS_NAME_ET)
        if (etList.isNullOrEmpty()) return
        AutoServiceUtils.setText(etList[0], ConfigManager.instance.getGreetWord())
        sleep(1500)

        val ivList = AutoServiceUtils.findViewByClassName(service, Constants.CLASS_NAME_IV)
        ...
        if (AutoServiceUtils.clickView(sendIv)) {
            sleep(ConfigManager.instance.getCommentInterval())
            if (isLive) {
                AutoServiceUtils.goBack(service)
                sleep(2000)
            }
            AutoServiceUtils.scrollViewVertical(
                service,
    ...
    private suspend fun commentLiveSquare() {
        val tvSayList = AutoServiceUtils.findViewByContainsText(service, "说点什么...")
        if (tvSayList.isNullOrEmpty()) {
            ...
            sleep(4000)
            commentLiveSquare()
        } else if (AutoServiceUtils.clickView(tvSayList[0])) {
            sleep(1500)
            val etList =
                AutoServiceUtils.findViewByClassName(service, Constants.CLASS_NAME_ET)
            if (etList.isNullOrEmpty()) return
            ...
            sleep(2000)
            val ivList =
                AutoServiceUtils.findViewByClassName(service, Constants.CLASS_NAME_IV)
            if (ivList.isNullOrEmpty() || ivList.size < 2) {
                return
            }
            if (AutoServiceUtils.clickView(ivList[1])) {
                sleep(1000)
                AutoServiceUtils.goBack(service)
                sleep(1500)
                val vpList =
...
//        val rvList = AutoServiceUtils.findViewByClassName(service, Constants.CLASS_NAME_RV)
////        if (rvList.isNullOrEmpty())
////            return
//        var rv = rvList[0]
////        rv = AutoServiceUtils.findViewById(service,"com.ss.android.ugc.aweme:id/b0u")
//        Log.e("---", "rv child ${rv.childCount}")
//        for (i in 0 until rv.childCount) {
//            val child = rv.getChild(i)
//            if (child == null) continue
//            AutoServiceUtils.clickView2(child)
//            break
//        }
        AutoServiceUtils.clickByGesture(
        ...
        if (AutoServiceUtils.clickView(ivList[2])) {
            sleep(1500)
            AutoServiceUtils.scrollViewVertical(
                service,
                UiUtils.getScreenWidth(App.getInstance()) / 4,
    ...
    private suspend fun commentRecommendVideo() {
        val commentButton =
            AutoServiceUtils.findViewByFirstContainsContentDescription(service, "评论")
        if (AutoServiceUtils.clickView(commentButton)) {
            sleep(1500)
            ...
            AutoServiceUtils.setText(commentEditText, ConfigManager.instance.getGreetWord())
            AutoServiceUtils.clickView(commentEditText)
            sleep(1500)
            val ivList =
                AutoServiceUtils.findViewByClassName(service, Constants.CLASS_NAME_IV)
            Log.e("-----", "ivList ${ivList.size}")
            if (ivList.size < 3) return
            AutoServiceUtils.clickView(ivList[2])
            sleep(1500)
            AutoServiceUtils.goBack(service)
            sleep(1000)
        }
    ...
    private suspend fun sendWordAndPicture(isWord: Boolean, isPicture: Boolean) {
        ...
        rv?.let {
            for (i in 0 until it.childCount) {
                ...
                Log.e("----", "child $i is null ${child == null}")
                child ?: continue
                val ivList = AutoServiceUtils.findViewByContentDescription(child, "发消息")
                Log.e("----", "ivList $i size ${ivList.size}")
                if (ivList.isNullOrEmpty()) continue
                AutoServiceUtils.clickView(ivList[0])
                sleep(1500)
                ...
                if (isWord) {
                    val editText =
                        AutoServiceUtils.findViewByFirstClassName(service, Constants.CLASS_NAME_ET)
                    editText?.let { et ->
                        AutoServiceUtils.setText(et, ConfigManager.instance.getGreetWord())
                        sleep(1500)
                    }
                    val send =
                        AutoServiceUtils.findViewByFirstEqualsContentDescription(service, "发送")
                    send?.let { sd ->
                        AutoServiceUtils.clickView(sd)
                        sleep(2500)
                    }
                }
                if (isPicture) {
                    ...
                    sleep(1500)
                    val tvPhoto = AutoServiceUtils.findViewByEqualsText(service, "照片")
                    if (!tvPhoto.isNullOrEmpty()) {
                        AutoServiceUtils.clickView(tvPhoto[0])
                        sleep(2000)
                        val rvPhotoList =
                            AutoServiceUtils.findViewByClassName(service, Constants.CLASS_NAME_RV)
                        rvPhotoList?.get(0)?.getChild(0)?.takeIf { it.childCount >= 2 }?.apply {
                            AutoServiceUtils.clickView(getChild(1))
                            sleep(2000)
                            AutoServiceUtils.findViewByContainsText(service, "发送")?.let {
                                AutoServiceUtils.clickView(it[0])
                                sleep(2000)
                                AutoServiceUtils.goBack(service)
                                sleep(2000)
    ...
    private suspend fun messageCommand() {
        ...
        for ((i, child) in childList.withIndex()) {
            ...
            AutoServiceUtils.setText(editText, ConfigManager.instance.getGreetWord())
            sleep(1500)
            val send =
                AutoServiceUtils.findViewByFirstEqualsContentDescription(service, "发送") ?: continue
            AutoServiceUtils.clickView(send)
            sleep(1500)
    ...
    private suspend fun trainAccountCommand() {
        var trainConfig = ConfigManager.instance.getControlConfig().trainConfig
        val viewPager =
            AutoServiceUtils.findViewByClassName(service, Constants.CLASS_NAME_VP)
        if (!viewPager.isNullOrEmpty()) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                while (true) {
                    AutoServiceUtils.doubleClickView(
                        service,
                    ...
                    if (AutoServiceUtils.clickView(commentButton)) {
                        ...
                        sleep(1000)
                        viewPager[0]?.performAction(AccessibilityNodeInfo.ACTION_SCROLL_FORWARD)
                        sleep(trainConfig.videoSwitchTime)
                    }
                }
            }
        }
    ...
    private suspend fun pariseCurrentWorkCommenter() {
        ...
        rv?.let {
            for (i in 0 until it.childCount) {
                val child = it.getChild(i) ?: continue
                val starView =
                    AutoServiceUtils.findViewByClassName(child, Constants.CLASS_NAME_VIEW)
                if (!starView.isNullOrEmpty()) {
                    AutoServiceUtils.clickView(starView[0])
                    sleep(1800)
                }
    ...
    private suspend fun privateMessagesCommand() {
        while (true) {
            ...
            if (starButton == null) {
                ...
                if (!viewPager.isNullOrEmpty()) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
//                        AutoServiceUtils.scrollView(service, viewPager.get(0))
                        viewPager.get(0)
...
//        }
//        AutoServiceUtils.clickView(entrance)
//        sleep(3000)

        AutoServiceUtils.scrollViewHorizontal(
            service,
        ...
        val moreButton =
            AutoServiceUtils.findViewByFirstEqualsContentDescription(service, "更多")
        var starTvList = AutoServiceUtils.findViewByContainsText(service, "关注")
    ...
    suspend fun toggleBatchStar(isStar: Boolean) {
        ...
        if (rvList.isNullOrEmpty() || rvList.size < 2) return
        val rv: AccessibilityNodeInfo
        val fansBarList = AutoServiceUtils.findViewByContainsText(service, "粉丝")
        ...
        if (rv != null) {
            ...
            for (i in 0 until rv.childCount) {
                val child = rv.getChild(i) ?: continue
                var listStar =
                    AutoServiceUtils.findViewByEqualsText(child, if (isStar) "关注" else "已关注")
                if (listStar.isNullOrEmpty()) {
                    listStar =
                        AutoServiceUtils.findViewByEqualsText(child, if (isStar) "回关" else "互相关注")
                }
                if (!listStar.isNullOrEmpty()) {
                    AutoServiceUtils.clickView(listStar[0])
                    sleep(1800)
